	...
	lastdrawcount(0)
{
	...
	setImageCount(core::dimension2d<u32>(4, 2));
...
const core::dimension2d<u32>& CGrassPatchSceneNode::getImageCount() const { return imagecount; }

void CGrassPatchSceneNode::setImageCount(const core::dimension2d<u32>& ic)
{
	imagecount = ic;
...
bool CGrassPatchSceneNode::Create()
{
	...
	for(i = 0; i < count; ++i)
	{
		...
		core::matrix4 m2;
		m2.setRotationDegrees(core::vector3df(0.0f, rotation, 0.0f));
...
void CGrassPatchSceneNode::render()
{
	...
	if (!redrawnextloop)
	{
		...
		driver->drawIndexedTriangleList(Vertices.pointer(), lastdrawcount*4, Indices.pointer(), lastdrawcount*4);
	}
	else
	{
		...
		for (u32 i = 0; i < max; ++i)
		{
			...
			const video::SColor gcol(
				particle.color.getAlpha(),
				(u32)(particle.color.getRed()*0.8f),
				(u32)(particle.color.getGreen()*0.8f),
				(u32)(particle.color.getBlue()*0.8f));
			...
			const u32 arrpos = (imagecount.Width*particle.sprite.Height) + particle.sprite.Width;
		...
		driver->setTransform(video::ETS_WORLD, AbsoluteTransformation);
		driver->setMaterial(Material);
		driver->drawIndexedTriangleList(Vertices.pointer(), drawcount*4, Indices.pointer(), drawcount*4);
...
{
	if (Particles.size()*4 > Vertices.size() || Particles.size()*6 > Indices.size())
	{
		const u32 oldSize = Vertices.size();
		...
		const u32 oldIdxSize = Indices.size();
